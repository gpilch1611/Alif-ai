...
const defaultState = {
  ...
  persona: "",
  prayerLog: {},          // {"YYYY-MM-DD": ["Fajr","Dhuhr",...]}
  duasDone: {},
  scholarMode: false,
  ramadanMode: false,
  pairCode: "",
  pairBuddyCode: "",
  hifzMemorized: {},      // {"surah-ayat": true}
  pushEnabled: false,
  tajweedColor: false
};
...
function aiSystemPrompt() {
  if (state.scholarMode) return scholarPrompt[state.lang === "en" ? "en" : "pl"];
...
const _saveStateDebounced = debounce(saveStateNow, 800);
function saveState() {
  _saveStateDebounced();
}
...
function render() {
  ...
  if (typeof islamWrapped === 'function') views.islam = islamWrapped;
  // Sprint 3: onboarding gate on first launch
  if (!state.onboardingDone && route === "home") {
    onboardingView();
    return;
  }
  (views[route] || home)();
...
async function loadAyatOfDay() {
  ...
  try {
    ...
    const [resAr, resPl, resEn] = await Promise.all([
      fetchTimeout(`https://api.alquran.cloud/v1/ayah/${randomAyah}`, {}, 8000),
      fetchTimeout(`https://api.alquran.cloud/v1/ayah/${randomAyah}/pl.bielawskiego`, {}, 8000),
      fetchTimeout(`https://api.alquran.cloud/v1/ayah/${randomAyah}/en.asad`, {}, 8000)
    ]);
...
async function openSurah(num) {
  ...
  try {
    ...
    const [resAudio, resTrans, resTr] = await Promise.all([
      fetchTimeout(`https://api.alquran.cloud/v1/surah/${num}/${reciter}`, {}, 12000),
      fetchTimeout(`https://api.alquran.cloud/v1/surah/${num}/${transEdition}`, {}, 12000),
      fetchTimeout(`https://api.alquran.cloud/v1/surah/${num}/en.transliteration`, {}, 12000)
    ]);
    ...
    if (dataAudio.code === 200) {
        ...
        <div class="panel p-5 sm:p-8">
          <div class="mb-6 text-center">
            ...
            <div class="mt-4 flex justify-center gap-2 flex-wrap">
               ...
               <button id="toggleTajweed" data-testid="toggle-tajweed-btn" class="big-action ${state.tajweedColor?'bg-amber-500 text-white':'bg-[var(--surface)]'}">${state.tajweedColor?'🎨 Tajweed ON':'🎨 Tajweed'}</button>
            </div>
          </div>
          <div class="grid gap-4">
            ${s.ayahs.map(ayah => {
              const ayahKey = `${s.number}:${ayah.numberInSurah}`;
              const tafsir = tafsirSnippets[ayahKey];
              const memorized = state.hifzMemorized?.[ayahKey];
              return `
              <div class="soft-panel p-4 ayah-card ${memorized?'ring-2 ring-emerald-400':''}" data-ayah-num="${ayah.number}" data-ayah-key="${ayahKey}" data-ayah-text="${escapeHtml(ayah.text)}" data-ayah-tr="${escapeHtml(trMap[ayah.numberInSurah] || "")}" data-ayah-trans="${escapeHtml(transMap[ayah.numberInSurah] || "")}">
                <div class="flex items-start justify-between gap-4 mb-2">
                  <span class="text-xs font-black text-emerald-600 mt-1 shrink-0">${ayah.numberInSurah}</span>
                  <p class="arabic text-right text-2xl sm:text-3xl leading-loose ${state.tajweedColor?'tajweed-on':''}">${state.tajweedColor ? tajweedColorize(ayah.text) : escapeHtml(ayah.text)}</p>
                </div>
                ...
                ${tafsir ? `<div class="tafsir-snippet mt-2"><strong>📖 Tafsir:</strong> ${escapeHtml(state.lang==='pl'?tafsir.pl:tafsir.en)}</div>` : ""}
                <div class="mt-3 flex justify-end gap-2 flex-wrap">
                  ...
                  <button class="speaker-btn haptic-feedback ${memorized?'bg-emerald-200':''}" data-hifz="${ayahKey}" data-testid="hifz-toggle-${ayahKey}" title="${tx("Zapamiętany / Hifz", "Memorized / Hifz")}">${memorized?'🧠✓':'🧠'}</button>
                </div>
              </div>
            `}).join("")}
          </div>
      ...
      container.querySelectorAll("[data-hifz]").forEach(btn => btn.addEventListener("click", () => {
        const k = btn.dataset.hifz;
        if (!state.hifzMemorized) state.hifzMemorized = {};
        if (state.hifzMemorized[k]) delete state.hifzMemorized[k];
        else { state.hifzMemorized[k] = true; addPoints(5, false); }
        saveStateNow();
        openSurah(s.number);
      }));

      $("#toggleTajweed")?.addEventListener("click", () => {
        state.tajweedColor = !state.tajweedColor;
        saveStateNow();
        openSurah(s.number);
      });
...
function settings() {
  view.innerHTML = `
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <section class="panel p-5 lg:col-span-2" data-testid="advanced-modes">
        <h2 class="text-xl font-black">${tx("Tryby zaawansowane", "Advanced modes")}</h2>
        <p class="text-sm text-[var(--muted)] mt-1">${tx("Ramadan, AI Scholar, tryb pary, powiadomienia.", "Ramadan, AI Scholar, pair mode, notifications.")}</p>
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
          <button id="toggleRamadan" data-testid="toggle-ramadan-btn" class="big-action ${state.ramadanMode?'bg-amber-500 text-white':'border border-[var(--line)] bg-[var(--surface)]'}">🌙 ${state.ramadanMode?tx('Ramadan Mode: ON','Ramadan Mode: ON'):tx('Ramadan Mode: OFF','Ramadan Mode: OFF')}</button>
          <button id="toggleScholar" data-testid="toggle-scholar-btn" class="big-action ${state.scholarMode?'bg-emerald-700 text-white':'border border-[var(--line)] bg-[var(--surface)]'}">🎓 ${state.scholarMode?tx('AI Scholar: ON','AI Scholar: ON'):tx('AI Scholar: OFF','AI Scholar: OFF')}</button>
          <button id="enablePush" data-testid="toggle-push-btn" class="big-action ${state.pushEnabled?'bg-blue-500 text-white':'border border-[var(--line)] bg-[var(--surface)]'}">🔔 ${state.pushEnabled?tx('Powiadomienia modlitw: ON','Prayer notifications: ON'):tx('Włącz powiadomienia modlitw','Enable prayer notifications')}</button>
          <button id="genPairCodeBtn" data-testid="gen-pair-code-btn" class="big-action border border-[var(--line)] bg-[var(--surface)]">💕 ${state.pairCode ? tx('Mój kod: ', 'My code: ') + state.pairCode : tx('Wygeneruj kod pary', 'Generate pair code')}</button>
        </div>
        <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          <input id="pairBuddyInput" data-testid="pair-buddy-input" type="text" placeholder="${tx('Kod partnera/ki (6 znaków)','Buddy code (6 chars)')}" value="${escapeHtml(state.pairBuddyCode||'')}" class="rounded-lg border border-[var(--line)] bg-[var(--surface)] p-3 uppercase tracking-widest" maxlength="6" />
          <button id="savePairBuddy" data-testid="save-pair-buddy-btn" class="big-action bg-rose-500 text-white">${tx('Zapisz partnera/kę','Save buddy')}</button>
        </div>
        <p class="text-xs text-[var(--muted)] mt-2">${tx("Pair Mode: 2 osoby (Borzęta ↔ Surabaya) widzą postępy nawzajem. Funkcja oparta na localStorage — działa offline.","Pair Mode: 2 people (Borzęta ↔ Surabaya) see each others progress. localStorage-based — works offline.")}</p>
      </section>
  ...
  $("#toggleRamadan")?.addEventListener("click", () => {
    state.ramadanMode = !state.ramadanMode;
    saveStateNow();
    showLoveToast(state.ramadanMode ? tx("🌙 Ramadan Mode włączony","🌙 Ramadan Mode enabled") : tx("Ramadan Mode wyłączony","Ramadan Mode disabled"));
    render();
  });
  $("#toggleScholar")?.addEventListener("click", () => {
    state.scholarMode = !state.scholarMode;
    saveStateNow();
    showLoveToast(state.scholarMode ? tx("🎓 AI Scholar persona aktywna","🎓 AI Scholar persona active") : tx("AI Scholar wyłączony","AI Scholar disabled"));
    render();
  });
  $("#enablePush")?.addEventListener("click", async () => {
    if (!('Notification' in window)) { showLoveToast(tx("Powiadomienia niedostępne na tym urządzeniu","Notifications unsupported on this device")); return; }
    const perm = await Notification.requestPermission();
    if (perm === 'granted') {
      state.pushEnabled = true;
      saveStateNow();
      schedulePrayerNotifications();
      showLoveToast(tx("🔔 Powiadomienia modlitw włączone","🔔 Prayer notifications enabled"));
      render();
    } else {
      showLoveToast(tx("Odmówiono pozwolenia","Permission denied"));
    }
  });
  $("#genPairCodeBtn")?.addEventListener("click", () => {
    state.pairCode = state.pairCode || genPairCode();
    saveStateNow();
    if (navigator.clipboard) navigator.clipboard.writeText(state.pairCode);
    showLoveToast(tx("Twój kod skopiowany: ","Your code copied: ") + state.pairCode);
    render();
  });
  $("#savePairBuddy")?.addEventListener("click", () => {
    const code = ($("#pairBuddyInput").value || '').trim().toUpperCase();
    state.pairBuddyCode = code;
    saveStateNow();
    showLoveToast(tx("💕 Partner/ka zapisany/a: ","💕 Buddy saved: ") + code);
  });
...
async function fetchPrayerTimes(loc) {
  ...
  const cacheKey = `prayer-times-${loc.id}-${y}-${m}-${d}`;
  return cached(cacheKey, 86400000, async () => {
...
async function fetchQibla(loc) {
  const cacheKey = `qibla-${loc.id}`;
  return cached(cacheKey, 30 * 86400000, async () => {
...
// ====================================================================
// SPRINT 3-5 NEW VIEWS — onboarding, fiqh, duas, prayer tracker, calendar, myths
// ====================================================================

function onboardingView() {
  const list = personas[state.lang] || personas.pl;
  view.innerHTML = sanitize(`
    <section class="space-y-4 max-w-2xl mx-auto" data-testid="onboarding-view">
      <div class="rounded-2xl bg-[var(--surface-soft)] border border-[var(--line)] p-6 text-center">
        <h1 class="text-3xl font-black tracking-tight mb-2">${tx('Witaj w Alif AI ✨','Welcome to Alif AI ✨')}</h1>
        <p class="text-[var(--muted)]">${tx('Wybierz, kim jesteś — spersonalizujemy Twój start.','Choose who you are — we will personalise your start.')}</p>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ${list.map(p => `<button data-persona="${p.id}" data-route="${p.start}" data-testid="persona-${p.id}" class="text-left rounded-xl border-2 border-[var(--line)] hover:border-emerald-400 bg-[var(--surface)] p-4 transition"><div class="text-3xl mb-1">${p.icon}</div><div class="font-bold">${p.title}</div><div class="text-sm text-[var(--muted)]">${p.desc}</div></button>`).join('')}
      </div>
      <button id="skipOnboarding" data-testid="skip-onboarding-btn" class="w-full mt-4 rounded-lg border border-[var(--line)] py-3 text-sm text-[var(--muted)]">${tx('Pomiń','Skip')}</button>
    </section>
  `);
  view.querySelectorAll('[data-persona]').forEach(b => b.addEventListener('click', () => {
    state.persona = b.dataset.persona;
    state.onboardingDone = true;
    saveStateNow();
    setRoute(b.dataset.route || 'home');
  }));
...
function fiqhView() {
  const cards = fiqhCards[state.lang] || fiqhCards.pl;
  const scale = obligationScale[state.lang] || obligationScale.pl;
  view.innerHTML = sanitize(`
    <section class="space-y-5" data-testid="fiqh-view">
      <header class="rounded-2xl bg-emerald-50/40 dark:bg-emerald-900/20 p-5 border border-emerald-200">
        <h1 class="text-2xl font-black">${tx('Fiqh — fundamenty praktyki','Fiqh — practice fundamentals')}</h1>
        <p class="text-sm text-[var(--muted)]">${tx('Skala obowiązku, ghusl, tayammum, tarawih, witr i sunan rawatib.','Scale of obligation, ghusl, tayammum, tarawih, witr and sunan rawatib.')}</p>
      </header>

      <div class="rounded-xl bg-[var(--surface-soft)] border border-[var(--line)] p-4">
        <h2 class="font-bold text-lg mb-3">${tx('Skala obowiązku w islamie','Scale of obligation in Islam')}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          ${scale.map(s => `<div class="rounded-lg border border-[var(--line)] p-3 bg-[var(--surface)]"><div class="font-bold">${s.icon} ${s.rank}</div><div class="text-sm">${s.desc}</div><div class="text-xs text-[var(--muted)] mt-1"><em>${s.sample}</em></div></div>`).join('')}
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ${cards.map(c => `<article class="rounded-xl bg-[var(--surface-soft)] border border-[var(--line)] p-4" data-testid="fiqh-${c.id}"><h3 class="font-bold text-lg">${c.icon} ${c.title}</h3><p class="text-sm mt-2">${c.body}</p><p class="text-xs text-emerald-700 mt-2"><strong>${tx('Kiedy:','When:')}</strong> ${c.when}</p></article>`).join('')}
      </div>
    </section>
  `);
}
...
function duasView() {
  ...
  view.innerHTML = sanitize(`
    <section class="space-y-4" data-testid="duas-view">
      <header class="rounded-2xl bg-[var(--surface-soft)] border border-[var(--line)] p-5">
        <h1 class="text-2xl font-black">${tx("Biblioteka du'a","Du'a library")}</h1>
        <p class="text-sm text-[var(--muted)]">${tx('Codzienne, w podróży, przy chorobie, ochronne.','Daily, travel, illness, protection.')}</p>
      </header>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ${expandedDuas.map(d => `<article class="rounded-xl bg-[var(--surface)] border border-[var(--line)] p-4" data-testid="dua-${d.id}"><div class="text-2xl mb-1 text-right" dir="rtl">${d.ar}</div><div class="text-sm italic text-[var(--muted)]">${d.tr}</div><div class="font-bold mt-2">${lang==='pl'?d.titlePl:d.titleEn}</div><div class="text-sm">${lang==='pl'?d.plPl:d.enEn}</div><div class="text-xs text-emerald-700 mt-1">${d.ref||''}</div></article>`).join('')}
      </div>
    </section>
  `);
}
...
function trackerView() {
  const day = today();
  const todayLog = state.prayerLog[day] || [];
  view.innerHTML = sanitize(`
    <section class="space-y-4" data-testid="tracker-view">
      <header class="rounded-2xl bg-emerald-100/60 dark:bg-emerald-900/30 border border-emerald-300 p-5">
        <h1 class="text-2xl font-black">${tx('Tracker modlitw','Prayer Tracker')}</h1>
        <p class="text-sm">${tx('Zaznacz, które modlitwy odprawiłeś/aś dziś.','Mark which prayers you completed today.')}</p>
      </header>
      <div class="grid grid-cols-5 gap-2" data-testid="prayer-buttons">
        ${PRAYER_KEYS.map(k => `<button data-prayer="${k}" class="rounded-xl border-2 ${todayLog.includes(k)?'border-emerald-500 bg-emerald-100 dark:bg-emerald-900':'border-[var(--line)] bg-[var(--surface)]'} p-3 font-bold" data-testid="prayer-${k}">${k}<div class="text-xl">${todayLog.includes(k)?'✓':'○'}</div></button>`).join('')}
      </div>
      <div class="rounded-xl bg-[var(--surface-soft)] border border-[var(--line)] p-4">
        <h2 class="font-bold mb-3">${tx('Ostatnie 7 dni','Last 7 days')}</h2>
        <div class="grid grid-cols-7 gap-1 text-center text-xs">
          ${Array.from({length:7}, (_,i) => {
            const d = new Date(Date.now() - (6-i)*86400000).toISOString().slice(0,10);
            const c = (state.prayerLog[d]||[]).length;
            const color = c===5?'bg-emerald-500 text-white':c>=3?'bg-emerald-300':c>=1?'bg-amber-200':'bg-[var(--surface)]';
            return `<div class="${color} rounded p-2 border border-[var(--line)]"><div class="font-bold">${d.slice(8)}</div><div>${c}/5</div></div>`;
          }).join('')}
        </div>
      </div>
    </section>
  `);
  view.querySelectorAll('[data-prayer]').forEach(btn => btn.addEventListener('click', () => {
    const k = btn.dataset.prayer;
    const log = state.prayerLog[day] || [];
    const idx = log.indexOf(k);
    if (idx >= 0) log.splice(idx, 1); else log.push(k);
    state.prayerLog[day] = log;
    saveStateNow();
    if (log.length === 5) { addPoints(15, false); confetti(); }
    trackerView();
  }));
}

function gregorianToHijriEvent(gDate) {
  // simple approximation already in app
  const h = gregorianToHijri(gDate);
  return hijriEvents.find(e => e.month === h.month && e.day === h.day);
}

function calendarView() {
  const today_evt = gregorianToHijriEvent(new Date());
  view.innerHTML = sanitize(`
    <section class="space-y-4" data-testid="calendar-view">
      <header class="rounded-2xl bg-[var(--surface-soft)] border border-[var(--line)] p-5">
        <h1 class="text-2xl font-black">${tx('Kalendarz islamski','Islamic Calendar')}</h1>
        ${today_evt ? `<p class="text-sm mt-2"><strong>${tx('Dziś:','Today:')}</strong> ${state.lang==='pl'?today_evt.namePl:today_evt.nameEn} — ${state.lang==='pl'?today_evt.notePl:today_evt.noteEn}</p>` : ''}
      </header>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ${hijriEvents.map(e => `<article class="rounded-xl bg-[var(--surface)] border border-[var(--line)] p-4"><div class="text-xs text-[var(--muted)]">${e.month}/${e.day} Hijri</div><h3 class="font-bold text-lg">${state.lang==='pl'?e.namePl:e.nameEn}</h3><p class="text-sm">${state.lang==='pl'?e.notePl:e.noteEn}</p></article>`).join('')}
      </div>
    </section>
  `);
}
...
function mythsView() {
  ...
  view.innerHTML = sanitize(`
    <section class="space-y-4" data-testid="myths-view">
      <header class="rounded-2xl bg-rose-50/60 dark:bg-rose-900/20 border border-rose-200 p-5">
        <h1 class="text-2xl font-black">${tx('Obalamy mity o islamie','Debunking myths about Islam')}</h1>
        <p class="text-sm text-[var(--muted)]">${tx('To, co mówią media, vs. co mówi Quran i hadis.','What the media say vs. what the Quran and hadith actually say.')}</p>
      </header>
      <div class="space-y-3">
        ${myths.map(m => `<details class="rounded-xl bg-[var(--surface-soft)] border border-[var(--line)] p-4" data-testid="myth-${m.id}"><summary class="font-bold cursor-pointer">${m.verdict==='false'?'❌':'⚖️'} ${state.lang==='pl'?m.qPl:m.qEn}</summary><p class="text-sm mt-3">${state.lang==='pl'?m.aPl:m.aEn}</p>${m.id==='suicide_bombing'?crisisNoteHTML(state.lang):''}<p class="text-xs text-emerald-700 mt-2">${m.ref||''}</p></details>`).join('')}
      </div>
    </section>
  `);
}

// Patch islam() & home() — wrap original by storing & redefining via globals on window
const _origIslam = islam;
window.__alifIslamWrapped = true;
function islamWrapped() {
  _origIslam();
  const islamView = $('#view');
  if (!islamView) return;
...
function homeWrapped() {
  ...
  homeExtras.querySelectorAll('[data-route]').forEach(b => b.addEventListener('click', () => setRoute(b.dataset.route)));
}

// ── Sprint 5: Push notifications for prayers ───────────────
...
async function schedulePrayerNotifications() {
  ...
  try {
    const loc = PRAYER_LOCATIONS[0];
    const timings = await fetchPrayerTimes(loc);
    if (!timings) return;
    const now = new Date();
    const today_ymd = now.toISOString().slice(0,10);
    PRAYER_KEYS.forEach(key => {
      const tStr = timings[key];
      if (!tStr) return;
      const [hh, mm] = tStr.split(':');
      const target = new Date();
      target.setHours(parseInt(hh), parseInt(mm) - 10, 0, 0); // 10 min przed
      const delta = target.getTime() - now.getTime();
      if (delta > 0 && delta < 86400000) {
        const id = setTimeout(() => {
          try {
            new Notification('🕌 Alif AI', {
              body: tx(`${key} za 10 minut (${tStr})`, `${key} in 10 minutes (${tStr})`),
              icon: '/assets/icon-192.png',
              tag: `prayer-${key}-${today_ymd}`
            });
          } catch {}
        }, delta);
        _prayerNotifTimers.push(id);
      }
    });
  } catch {}
}

// ── Sprint 5: Ramadan ribbon on home ──────────────────────
function ramadanRibbon() {
  if (!state.ramadanMode) return '';
  const now = new Date();
  const hijri = gregorianToHijri(now);
  const dayOfRamadan = hijri.month === 9 ? hijri.day : null;
  const last10 = dayOfRamadan && dayOfRamadan >= 21;
  return sanitize(`<div class="rounded-2xl bg-gradient-to-r from-amber-400 to-amber-600 text-white p-5 mb-4 shadow-lg" data-testid="ramadan-ribbon">
    <div class="text-2xl font-black">🌙 ${tx('Ramadan Mode aktywny', 'Ramadan Mode active')}</div>
    ${dayOfRamadan ? `<div class="text-sm">${tx('Dzień Ramadanu:', 'Day of Ramadan:')} <strong>${dayOfRamadan}/30</strong>${last10?' — '+tx('Ostatnie 10 nocy! Szukaj Laylat al-Qadr.','Last 10 nights! Seek Laylat al-Qadr.'):''}</div>` : `<div class="text-sm">${tx('Tryb przygotuje Cię do Ramadanu.', 'Mode prepares you for Ramadan.')}</div>`}
    <div class="mt-2 text-xs">${tx('Plan: 1 juz Quranu/dzień · Tarawih · dhikr · sadaqa.', 'Plan: 1 juz/day · Tarawih · dhikr · sadaqa.')}</div>
  </div>`);
}

// ── Sprint 5: Pair Mode buddy widget ──────────────────────
function pairBuddyWidget() {
  if (!state.pairCode && !state.pairBuddyCode) return '';
  const myDone = (state.prayerLog[today()] || []).length;
  return sanitize(`<div class="rounded-2xl bg-gradient-to-r from-rose-300 to-pink-400 text-white p-4 mt-4" data-testid="pair-buddy-widget">
    <div class="flex items-center justify-between">
      <div>
        <div class="font-black">💕 ${tx('Tryb pary', 'Pair Mode')}</div>
        <div class="text-xs opacity-90">Borzęta ↔ Surabaya</div>
      </div>
      <div class="text-right text-sm">
        ${state.pairCode?`<div>${tx('Mój kod:','My code:')} <strong class="font-mono">${escapeHtml(state.pairCode)}</strong></div>`:''}
        ${state.pairBuddyCode?`<div>${tx('Kod partnera/ki:','Buddy:')} <strong class="font-mono">${escapeHtml(state.pairBuddyCode)}</strong></div>`:''}
      </div>
    </div>
    <div class="mt-2 text-xs">${tx('Dziś modlitw:','Prayers today:')} <strong>${myDone}/5</strong></div>
  </div>`);
}

// Patch home wrapper to include Ramadan ribbon + pair buddy
const _origHomeWrapped = homeWrapped;
function home2() {
  _origHomeWrapped();
  const v = $('#view');
  if (!v) return;
  if (state.ramadanMode) v.insertAdjacentHTML('afterbegin', ramadanRibbon());
  v.insertAdjacentHTML('beforeend', pairBuddyWidget());
}
...
function renderV2() {
  _renderOrig();
  // Schedule notifications opportunistically once per session
  if (state.pushEnabled && !window.__prayerNotifScheduled) {
    window.__prayerNotifScheduled = true;
