...
async function loadAyatOfDay() {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Use batched editions endpoint to reduce 3 network requests to 1.
    ...
    if (data.code === 200 && Array.isArray(data.data) && data.data.length >= 2) {
      const [dataAr, dataPl, dataEn] = data.data;
      const cache = {
        date: todayDate,
...
async function openSurah(num, options = {}) {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Use batched editions endpoint to fetch audio, translation, and transliteration in a single request.
    const res = await fetch(`https://api.alquran.cloud/v1/surah/${num}/editions/${reciter},${transEdition},en.transliteration`);
    const data = await res.json();

    if (data.code === 200 && Array.isArray(data.data) && data.data.length >= 1) {
      const [dataAudio, dataTrans, dataTr] = data.data;
      const s = dataAudio;
