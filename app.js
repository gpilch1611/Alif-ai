...
async function loadAyatOfDay() {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Batch multiple editions into a single request to reduce network waterfall (reduces 3 requests to 1)
    const res = await fetch(
      `https://api.alquran.cloud/v1/ayah/${randomAyah}/editions/quran-simple,pl.bielawskiego,en.asad`
    );
...
async function openSurah(num, options = {}) {
  ...
  try {
    ...
    // BOLT OPTIMIZATION: Batch multiple editions into a single request to reduce network waterfall (reduces 3 requests to 1)
    const res = await fetch(
      `https://api.alquran.cloud/v1/surah/${num}/editions/${reciter},${transEdition},en.transliteration`
    );
    const result = await res.json();
